...
> **Ders:** BIM 320 Görüntü İşleme | **Öğretim Üyesi:** Hasibe Büşra AYTEKİN
...
- TensorFlow, PyTorch, Keras, pretrained weights
...
Dataset iki farklı Roboflow kaynağından kalite filtrelenerek oluşturulmuştur:

| Kaynak | Ham Görsel | Filtreleme | Final'e Giren |
|---|---|---|---|
| **V1** (Roboflow COCO split) | 2.072 | Boş maske → 806, ardından IoU > %70 filtresi | **370** |
| **V3+V4** (bitti — hazır maskeli set) | 1.002 | IoU > %20 kalite filtresi (45 hatalı maske elendi) | **957** |
| **Toplam** | | | **1327** |

> **Filtre kararları:**
> - V1: Algoritmanın en az %70 başarı gösterdiği görseller alındı — boş veya hatalı GT maskesi taşıyan görseller etkiyi kirletmesin diye elendi.
> - V3+V4: Algoritmanın sıfır veya neredeyse sıfır tespit ettiği 45 görsel elendi; bunlar yanlış/boş maske taşıyan örneklerdi.

### Klasör Yapısı

```
dataset/
├── images/    (1327 adet .jpg — 640×640 piksel)
└── masks/     (1327 adet .png — binary, 0/255)
```
...
```
final_fire_segmentation_project/
├── dataset/
│   ├── images/    (1327 × .jpg — 640×640)
│   └── masks/     (1327 × .png — binary)
├── src/
...
├── models/
│   └── README.md    (klasik CV — model ağırlık dosyası yoktur)
└── README.md
```
...
- **Hız:** Tek görsel için ortalama ~50–100ms (CPU, 640×640).
